...
def generate_link_state_vector(fwd_table, route_topology):
    encoded_vector = []
    for entry, path in route_topology.items():
        path_str = ""
        if path:
            path_str = "+".join((each_path_entry[0] + "," + each_path_entry[1]) for each_path_entry in path)
        matched_fwd_entry = any((fwd_entry[DESTINATION] == entry for fwd_entry in fwd_table))
        encoded_vector.append(entry[DESTINATION][0] + ":" + str(entry[DESTINATION][1]) + ":" + str(matched_fwd_entry[COST]) + ":" + path_str)
...
def decode_link_state_vector(packet_inner_data):
    ...
    for each_pair in ip_port_cost_pairs:
        dest_ip_addr, dest_port, dest_cost, path_str = each_pair.split(":")
        ip_port_strs = path_str.split("+")
        all_ip_port_paths = ip_port_strs.split(",")
